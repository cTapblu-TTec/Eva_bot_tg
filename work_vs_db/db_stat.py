...
class StatDatabase:
    ...
    async def create(self, pool: asyncpg.Pool):
        ...
        query = """CREATE TABLE IF NOT EXISTS public.statisticvk
                (
                    user_name character varying(30) COLLATE pg_catalog."default" NOT NULL,
                    CONSTRAINT "Statisticvk_pkey" PRIMARY KEY (user_name)
                ); """
        async with self.pool.acquire(): await self.pool.execute(query)

        query = """SELECT user_name FROM statisticvk;"""
    ...
    async def write(self, command: str, user_name: str):
        ...
        if self.users_st == [] or user_name not in self.users_st:
            query = """INSERT INTO statisticvk (user_name) VALUES ($1) ON CONFLICT (user_name) DO NOTHING;"""
        ...
        if stolbec not in self.columns:
            query = f'ALTER TABLE public.statisticvk ADD COLUMN IF NOT EXISTS {stolbec} smallint DEFAULT 0'
            async with self.pool.acquire(): await self.pool.execute(query)
            self.columns.append(stolbec)

        # ОБНОВЛЕНИЕ статистики пользователя
        query = f'UPDATE statisticvk SET {stolbec} = {stolbec} + 1 WHERE user_name = $1;'
        async with self.pool.acquire(): await self.pool.execute(query, user_name)

    # ______CLEAR______
    async def clear(self):
        query = 'DELETE FROM statisticvk *'
        async with self.pool.acquire(): await self.pool.execute(query)
        self.users_st = []
    ...
    async def dell(self):
        ...
        query = 'DROP TABLE IF EXISTS public.statisticvk'
        async with self.pool.acquire(): await self.pool.execute(query)
        self.columns = []
        self.users_st = []
