...
# n_last_shabl - номер последнего шаблона выданного пользователю, для списка ['N_50_sabl']
# SHABLON
...
async def shab(message: types.Message):
    ...
    text, vars_db.n_shablon, u.n_zamen, u.n_last_shabl = await Vid_Shabl(vars_db.n_shablon, u.n_zamen, u.n_last_shabl)  # получаем шаблон
    text, vars_db.n_otmetki, u.n_last_otm, s.l_otm_vernuli = await Vid_Otmetok(text, vars_db.n_otmetki, k, s.l_otm_vernuli)  # получаем отметки

    await message.answer(text)

    await users_db.write(message.from_user.username, ['n_last_otm', 'n_zamen', 'n_last_shabl'], [u.n_last_otm, u.n_zamen, u.n_last_shabl])
...
async def stor(message: types.Message):
    ...
    text, vars_db.n_otmetki, u.n_last_otm, s.l_otm_vernuli = await Vid_Otmetok('', vars_db.n_otmetki, k, s.l_otm_vernuli)  # получаем отметки
    ...
    await log(f'№ отметки: {vars_db.n_otmetki}, № сторис: {vars_db.n_storis} ({message.from_user.username} - /storis)\n')
...
async def vernut(message: types.Message):
    ...
    if u.n_last_otm is None or u.n_last_otm == 0:
        ...
        for i in u.n_last_otm:
            x = i.find('@')
            y = i.find('\\')
            if y != -1: list_o.append(i[x:y])
            else: list_o.append(i[x:])
