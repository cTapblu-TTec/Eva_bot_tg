...
async def adm_exit(message: types.Message):
    ...
    for disk in part:
        try:
            ...
            text += f"использовано {hd_used:.4} из {hd_total:.4} Gb на диске {disk.device} ({hd.percent}%)\n"
        except:
            pass
    ...
    await message.answer(text +
                         f"использовано {mem_used:.4} из {mem_total:.4} Gb оперативки ({mem.percent}%)")
