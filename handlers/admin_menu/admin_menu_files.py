...
from utils.admin_menu_utils import delete_old_message, create_menu_back, create_menu_cancel, edit_message, \
    delete_all_after_time, delete_loue_level_menu, create_level_menu

from loader import dp
from filters.admin_filters import CallFilterForFileValue, CallFilterForDelFile, CallFilterForSendFile
...
@dp.callback_query_handler(CallFilterForDelFile(), state=FCM.waite_delete_file)
...
@dp.callback_query_handler(CallFilterForSendFile(), state=FCM.waite_file_for_send)
async def send_file_2(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        if file_name in f_db.files:
            ...
            await log.write(f"admin_menu: файл '{file_name}' отправлен, ({call.from_user.username})")
        else:
